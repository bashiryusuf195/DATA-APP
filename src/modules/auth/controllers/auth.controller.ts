...
export async function updateProfileController(
  ...
  try {
    ...
    const { first_name, last_name, phone, username, date_of_birth, address_line1, gender } = req.body as Record<string, string>;
    ...
    if (gender !== undefined) {
      const g = String(gender).trim().toLowerCase() || null;
      const validGenders = ["male", "female", "other", "prefer_not_to_say"];
      if (g && !validGenders.includes(g)) {
        throw new AppError(422, "INVALID_GENDER", "Gender must be male, female, other, or prefer_not_to_say");
      }
