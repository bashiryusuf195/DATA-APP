...
export class VTPassProvider extends HttpVTUProvider {
  ...
  // ── Auth headers ──────────────────────────────────────────────────────────
  //
  // VTPass REST API authentication:
  //   GET  /balance                        → api-key + public-key
  //   POST /pay, /requery, /merchant-verify → api-key + secret-key

  // VTPass requires the first 8 characters of request_id to be today's date
  // (YYYYMMDD).
  //
  // Our internal references are typically like:
  // "DAT-20260611-XXXXXX"
  //
  // We strip the prefix and generate a Lagos-local timestamp prefix.
  ...
  private async fetchWithTimeout(
    ...
    try {
      ...
      if ((err as Error).name === "AbortError") {
        throw new Error(
          `VTPass request timed out after ${timeoutMs}ms [${url}]`
        );
      }
  ...
  private async parseJson<T>(
    ...
    try {
      ...
      throw new Error(
        `VTPass: could not read ${context} response body`
      );
    }
    ...
    try {
      ...
      throw new Error(
        `VTPass: non-JSON ${context} response (HTTP ${response.status}) — body: ${text.slice(
          0,
          200
        )}`
      );
  ...
  private normalizePurchaseResponse(
    ...
    const isSuccess =
      raw.code === CODE_SUCCESS || txn?.status === "delivered";

    const isPending =
      raw.code === CODE_PENDING || txn?.status === "initiated";
  ...
  private buildDataPayload(
    ...
    const variationCode =
      input.provider_variation_code ?? input.variation_code;

    if (!variationCode) {
      throw new Error(
        "VTPass data purchase requires variation_code (data plan code)"
      );
    }
    ...
    const base = input.network_operator
      .replace(/-data$/i, "")
      .toLowerCase();
  ...
  private buildCableTvPayload(
    ...
    if (!input.smartcard_number) {
      throw new Error(
        "VTPass cable TV purchase requires smartcard_number (IUC number)"
      );
    }
    ...
    if (!variationCode) {
      throw new Error(
        "VTPass cable TV purchase requires variation_code (subscription package)"
      );
    }
  ...
  // DB mapping:
  //
  // variation_code          = yola-electric-prepaid
  // network_operator        = yola-electric
  // provider_variation_code = prepaid
  // plan_category           = electricity
  //
  // VTPass expects:
  //
  ...
  //
  // Therefore provider_variation_code MUST take priority here.
  private buildElectricityPayload(
    input: ProviderPurchaseInput
  ): Record<string, unknown> {
    if (!input.network_operator) {
      throw new Error(
    ...
    if (!input.meter_number) {
      throw new Error(
        "VTPass electricity purchase requires meter_number"
      );
    }

    const meterType =
      input.provider_variation_code ??
      input.variation_code ??
      "prepaid";
  ...
  private buildExamPinPayload(
    ...
    if (!input.variation_code) {
      throw new Error(
        "VTPass exam pin purchase requires variation_code"
      );
    }
  ...
  async purchase(
    ...
    console.log("[VTPASS] purchase →", {
      ...
      variation_code:
        payload["variation_code"] ?? "(none — airtime)",
      amount: input.amount,
    ...
    if (!response.ok) {
      throw new Error(
        `VTPass purchase failed with HTTP ${response.status}`
      );
    }

    const raw =
      await this.parseJson<VTPassPurchaseResponse>(
        response,
        "purchase"
      );
    ...
    if (AUTH_FAILURE_CODES.has(raw.code)) {
      throw new Error(
        `VTPass authentication error: ${
          raw.response_description ?? raw.code
        }`
      );
    }
    ...
    return this.normalizePurchaseResponse(
      raw,
      input.reference
    );
  }
  ...
  async verifyTransaction(
    ...
    const response = await this.fetchWithTimeout(url, {
      ...
      headers: this.writeHeaders(creds),
      body: JSON.stringify({
        request_id: this.vtpassRequestId(reference),
      }),
    });
    ...
    if (!response.ok) {
      throw new Error(
        `VTPass requery failed with HTTP ${response.status}`
      );
    }
    ...
    return {
      ...
      message:
        raw.response_description ?? "Requery completed",
      raw_response: raw,
    };
  }
  ...
  async getBalance(): Promise<ProviderBalance> {
    ...
    if (response.status === 401) {
      ...
      throw new Error(
        `VTPass: HTTP 401 on /balance — api-key prefix: ${creds.apiKey.slice(
          0,
          6
        )}... — VTPass said: ${body.slice(0, 300)}`
      );
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `VTPass balance check failed with HTTP ${response.status}: ${body.slice(
          0,
          200
        )}`
      );
    ...
    const raw =
      await this.parseJson<VTPassBalanceResponse>(
        response,
        "balance"
      );

    console.log(
      "[VTPASS] balance raw response:",
      JSON.stringify(raw)
    );
  ...
  async getServiceVariations(
    ...
    const url =
      `${creds.baseUrl}/service-variations?serviceID=` +
    ...
    if (!response.ok) {
      throw new Error(
        `VTPass /service-variations failed (HTTP ${response.status}): ${text.slice(
          0,
          300
        )}`
      );
    ...
    try {
      ...
      throw new Error(
        `VTPass /service-variations returned non-JSON: ${text.slice(
          0,
          300
        )}`
      );
  ...
  async healthCheck(): Promise<ProviderHealthResult> {
    ...
    if (!creds) {
      return {
        healthy: false,
        message:
          "VTPass credentials not configured — add base_url, api_key, secret_key, and public_key in Admin > API Integrations",
      };
    }
    ...
    if (!creds.base_url) {
      return {
        healthy: false,
        message:
          "VTPass base_url not set — add in Admin > API Integrations > VTPass",
      };
    }
    ...
    if (!creds.api_key_encrypted) {
      return {
        healthy: false,
        message:
          "VTPass api_key not set — add in Admin > API Integrations > VTPass",
      };
    }
    ...
    if (!creds.secret_key_encrypted) {
      return {
        healthy: false,
        message:
          "VTPass secret_key not set — add in Admin > API Integrations > VTPass",
      };
    }
    ...
    if (!publicKey) {
      return {
        healthy: false,
        message:
          "VTPass public_key not set — add in Admin > API Integrations > VTPass",
      };
    }
    ...
    try {
      ...
      if (msg.includes("timed out")) {
        return {
          ...
          latency_ms,
          message:
            "VTPass health check timed out — check base_url",
        };
      }
      ...
      if (
        ...
        return {
          ...
          latency_ms,
          message:
            "VTPass network unreachable — check base_url",
        };
      }
  ...
  async verifyMeter(
    input: MeterVerifyInput
  ): Promise<MeterVerifyResult> {
    const creds = await this.loadCreds();
    ...
    if (response.status === 401) {
      throw new Error(
        "VTPass: HTTP 401 on /merchant-verify — verify api_key in Admin > API Integrations > VTPass."
      );
    }

    if (!response.ok) {
      throw new Error(
        `VTPass meter verify failed with HTTP ${response.status}`
      );
    }

    const raw =
      await this.parseJson<VTPassMerchantVerifyResponse>(
        response,
        "merchant-verify"
      );
    ...
    if (raw.code !== CODE_SUCCESS) {
      return {
        ...
        message:
          raw.response_description ??
          `Meter verification failed (code: ${raw.code})`,
        raw_response: raw,
      };
    }
    ...
    return {
      ...
      address: c.Address,
      meter_number:
        c.MeterNumber ?? input.meter_number,
  ...
  async verifyCable(
    input: CableVerifyInput
  ): Promise<CableVerifyResult> {
    const creds = await this.loadCreds();

    console.log("[VTPASS] verifyCable →", {
      serviceID: input.biller_code,
      smartcard:
        input.smartcard_number.slice(0, 4) + "***",
    });
    ...
    if (!response.ok) {
      throw new Error(
        `VTPass cable verify failed with HTTP ${response.status}`
      );
    }
    ...
    if (raw.code !== CODE_SUCCESS) {
      return {
        ...
        message:
          raw.response_description ??
          `Cable verification failed (code: ${raw.code})`,
        raw_response: raw,
      };
    }
    ...
    return {
      ...
      due_date: c.Due_Date,
      smartcard_number:
        c.smartcard_number ?? input.smartcard_number,
